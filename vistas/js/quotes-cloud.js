...
document.addEventListener("DOMContentLoaded", () => {   
    if (window.location.pathname.endsWith('quotes-cloud')) {
        agregarFila();  // Agrega una fila por defecto al cargar la página en la sección "quotes-cloud".
        
        // Escuchar cambios en u-rack-bellet y u-rack-liray
        document.getElementById('u-rack-bellet').addEventListener('input', actualizarCostoTotal);
        document.getElementById('u-rack-liray').addEventListener('input', actualizarCostoTotal);
...
function actualizarCostoTotal() {
    ...
    // Suma los valores de u-rack-bellet y u-rack-liray multiplicados por su precio unitario
    let bellet = parseFloat(document.getElementById('u-rack-bellet').value) * sqlDCUBellet;
    let liray = parseFloat(document.getElementById('u-rack-liray').value) * sqlDCULiray;
    if (isNaN(bellet)) bellet = 0;
    if (isNaN(liray)) liray = 0;
...
/* Función que actualiza el "Valor Mes (USD)" de cada fila calculando el valor basado en los recursos de cada VM
...
function actualizarCostoCadaFila() {
    ...
    // Recorre todos los inputs y selects de la fila actual
    fila.querySelectorAll("input, select").forEach((element, index, array) => {
        let valor = 0;
        ...
        else if (element.tagName === "SELECT" && baseName === "bkp")
        {
            ...
            if (element.value === "Bronze") valorFila += sqlBkpBronze;
            else if (element.value === "Silver") valorFila += sqlBkpSilver;
            else if (element.value === "Gold") valorFila += sqlBkpGold;
            else if (element.value === "Platinum") valorFila += sqlBkpPlatinum;
        }
        ...
        // Asigna el valor al Combobox "SISTEMA OPERATIVO - SO"
            ...
            if (element.value === "Linux")  valorFila += sqlSOLinux;
            else if (element.value === "Windows") valorFila += sqlSOWindows;
            else if (element.value === "Oracle") valorFila += sqlSOOracle;
        }

        // Asigna el valor al Combobox "BASE DE DATOS - BD"
        else if (element.tagName === "SELECT" && baseName === "bd"){ 
            // Asignar el nuevo valor para la BD
            if (element.value === "MySQL") valorFila += sqlBDMySQL;
            else if (element.value === "SQLServer") valorFila += sqlBDSQLServer;
            else if (element.value === "PostgreSQL") valorFila += sqlBDPostgreSQL;
        }

        // Asigna el valor al Combobox "BW Internet Primario Bellet"
        else if (element.tagName === "SELECT" && baseName === "bwinetbp") {
            if (element.value === "100Mbps") valorFila += sqlRedesBWINET100PrimarioBellet;
            else if (element.value === "200Mbps") valorFila += sqlRedesBWINET200PrimarioBellet;
            else if (element.value === "300Mbps") valorFila += sqlRedesBWINET300PrimarioBellet;
        }

        // Asigna el valor al Combobox "BW Internet Secundario Bellet"
        else if (element.tagName === "SELECT" && baseName === "bwinetbs") {
            if (element.value === "100Mbps") valorFila += sqlRedesBWINET100SecundarioBellet;
            else if (element.value === "200Mbps") valorFila += sqlRedesBWINET200SecundarioBellet;
            else if (element.value === "300Mbps") valorFila += sqlRedesBWINET300SecundarioBellet;
        }

        // Asigna el valor al Combobox "BW Internet Primario Liray"
        else if (element.tagName === "SELECT" && baseName === "bwinetlp") {
            if (element.value === "100Mbps") valorFila += sqlRedesBWINET100PrimarioLiray;
            else if (element.value === "200Mbps") valorFila += sqlRedesBWINET200PrimarioLiray;
            else if (element.value === "300Mbps") valorFila += sqlRedesBWINET300PrimarioLiray;
        }

        // Asigna el valor al Combobox "BW Internet Secundario Liray"
        else if (element.tagName === "SELECT" && baseName === "bwinetls") {
            if (element.value === "100Mbps") valorFila += sqlRedesBWINET100SecundarioLiray;
            else if (element.value === "200Mbps") valorFila += sqlRedesBWINET200SecundarioLiray;
            else if (element.value === "300Mbps") valorFila += sqlRedesBWINET300SecundarioLiray;
        }

        /*...Calculando el valor de cada fila...*/
     
        //Multiplica el valorFila (Valor Mes USD) por Qty sólo en la última iteración del ForEach
        if (index === array.length - 1) {
            valorFila = valorFila * cantSrv; 
        }

        //Suma al valorFila, el valor de IP pública en caso de estar seleccionado
        valorFila += valorFilaCheckIP; 

        // Actualiza el input de valor de la fila
    ...
    columnas.forEach((columna) => {
        ...
        switch (columna) {
            ...
            case "valor":
                ...
                input.readOnly = true; // Esto deja bloqueado el valor mes para edición
